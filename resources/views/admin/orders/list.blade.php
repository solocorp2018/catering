                ...
                <div class="d-flex">
                    ...
                    <div class="col-lg-2">
                        <label>Payment Status 
                            ...
                            <option value="{{$status['name']}}" {{SELECT($status['name'],request('status'))}}>{{$status['name']}}</option>
                            @endforeach
                            </select>  
                            </label>
                     </div>
                     <div class="col-lg-2">
                        <label>Payment Type 
                            <select name="payment_type" class="form-control form-control-line searchable" onchange="searchFun()"> 
                              <option value="" >-- Select -- </option>             
                                @foreach($paymentMode as $mode)
                            <option value="{{$mode['id']}}" {{SELECT($mode['id'],request('payment_type'))}}>{{$mode['name']}}</option>
                            @endforeach
                            </select>  
                            </label>
                     </div>
                  ...
                  <div class="d-flex">                    
                     <div class="col-lg-2 mt-3">
                        <a href="{{urlWithQueryString('orders-export')}}" class="btn btn-info d-none d-lg-block"> <i class="fas fa-downlaod"></i>&nbsp;Export</a>
                     </div>
                        ...
                        <table class="table table-hover">
                           <thead>
                              <tr>
                                 ...
                                 <th><a class="sort" data-column="amount"><i class="fa fa-sort" aria-hidden="true"></i> &nbsp;Amount</a></th>
                                 <th>Items</th>
                                 <th>Session</th>
                                 <th>Session Code</th>
                                 <th>Payment</th>
                           ...
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td><span class="text-info">{{$result->order_unique_id ?? '--'}}</span></td>
                                 <td>{{dateOf($result->order_date) ?? ''}}</td>
                                 <td>{!! amount($result->total_amount) ?? 0 !!}</td>
                                 <td>{{$result->orderItems->count() ?? 0}}</td>
                                 <td>{{$result->sessionMenu->sessionType->type_name ?? '--'}}</td>
                                 <td>{{$result->sessionMenu->session_code ?? '--'}}</td>
                                 <td>@if(!empty($result->payment) && $result->payment->payment_status == 2) <span class="text-success">Paid</span> @else <span class="text-danger">Pending</span> @endif</td>
                                 <td>
                                    <a class="waves-effect waves-dark" href="{{route('orders.show',$result->id)}}" data-toggle="tooltip" data-original-title="View Order"><i class="fa fa-eye"></i></a>
                                    &nbsp;&nbsp;&nbsp;                      
                                    <a class="waves-effect waves-dark text-success" href="{{route('order.invoice',$result->id)}}" data-toggle="tooltip" data-original-title="Download Order Invoice"><i class="fa fa-download"></i></a>
                                    @if(empty($result->payment))
                                    &nbsp;&nbsp;&nbsp;
                                    <a class="waves-effect waves-dark text-info" href="#" data-id="{{$result->id}}" id="updatePayment{{$result->id}}" data-toggle="tooltip" data-original-title="Update Payment"><i class="fa fa-file"></i></a>
                                    @endif
                                 </td>
                              </tr>
                              @endforeach
