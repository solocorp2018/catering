                  ...
                  <div class="p-5 osahan-invoice bg-white shadow-sm">
                     <div class="row">
                     	<div class="col-md-6">
                           ...
                           <p class="mb-1 text-black">Order No: <strong>{{$result->order->order_unique_id}}</strong></p>
                           <p class="mb-1">Order placed at: <strong>{{$result->order->order_date}}</strong></p>
                           <p class="mb-4 mt-2">
                     	...
                     	<div class="col-md-6">
                           ...
                           <h6 class="mb-1 text-black">{{$result->paidBy->name ?? ''}}</h6>
                           <p class="mb-1">{{$result->order->deliveredAddress->address_line_1 ?? ''}},  {{$result->order->deliveredAddress->address_line_2 ?? ''}}, {{$result->order->deliveredAddress->city ?? ''}}, {{$result->order->deliveredAddress->pincode ?? ''}}</p>
                        </div>
                     </div>
                     <div class="row mt-2">
                        <div class="col-md-12">
                           <table class="table mt-1 mb-0">
                              ...
                              <tbody>
                                @foreach($result->order->orderItems as $key => $orderItems)
                                 <tr>
